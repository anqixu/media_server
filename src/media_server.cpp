...
// TODO: debug why if we can't load video device; or loads video file that's not there, ROS node crashes and service call does not return
...
public:
  MediaServer(ros::NodeHandle& nh,
    ...
    // Setup camera topics and services
    camName = nh.resolveName("camera");
    std::string imageName = nh.resolveName("image");
    image_transport::ImageTransport it(nh);
    camTopic = camName + '/' + imageName;
    camPub = it.advertiseCamera(camTopic, 1);
    setCamInfoSrv = nh.advertiseService(camName + "/set_camera_info",
      &MediaServer::setCamInfo, this);
    loadIntrinsicsFile();
    ...
    ROS_INFO_STREAM("MEDIA_SERVER: initiated idle server on camera topic " <<
        camTopic << " with idle mode " << (idleEnabled ? "ENABLED" : "DISABLED"));
  };
  ...
  void loadIntrinsicsFile() {
    if (intrinsicsPath.length() <= 0) {
      intrinsicsPath = string(getenv("HOME")) + "/.ros/camera_info/" + camName + ".yaml";
    }
    if (camera_calibration_parsers::readCalibration(intrinsicsPath, camName, camInfo)) {
      ROS_INFO_STREAM("Loaded intrinsics parameters for " << camName);
    }
  };
  
  bool saveIntrinsicsFile() {
    if (camera_calibration_parsers::writeCalibration(intrinsicsPath, camName, camInfo)) {
      ROS_INFO_STREAM("Saved intrinsics parameters for " << camName);
      return true;
    }
    return false;
  };
...
private:
  bool pushNextFrame() {
    ...
    if (hasFrame) {
      result = true;
      imageBuf.toImageMsg(camImage);
      camPub.publish(camImage, camInfo);
    }
  ...
  ros::ServiceServer setCamInfoSrv;
  ...
  std::string camName;
  std::string camTopic;
  std::string intrinsicsPath; // TODO: set via ros param
...
int main(int argc, char** argv) {
  ...
  if (nh.resolveName("image") == "/image") {
    ROS_WARN_STREAM("Usage: " << argv[0] <<
        " camera:=<camera namespace> image:=<image name> [idleAfterSourceDone customStartTime]");
  }
